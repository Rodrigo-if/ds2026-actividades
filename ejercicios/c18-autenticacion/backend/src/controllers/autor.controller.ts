...
export async function getLibrosByAutor(req: Request, res: Response) {
  const autorId = Number(req.params.id);
  const libros = await autoresService.findLibrosByAutorId(autorId);
  if (!libros) return res.status(404).json({ mensaje: "Autor no encontrado" });
  res.json(libros);
}